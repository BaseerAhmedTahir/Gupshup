...
import { X, UserPlus, Loader2, Mail, Users } from 'lucide-react';
...
const AddMemberModal: React.FC<AddMemberModalProps> = ({ 
  ...
  const [existingMembers, setExistingMembers] = useState<Contact[]>([]);
  ...
  const fetchContacts = async () => {
    ...
    try {
      ...
      if (data) {
        ...
        // Separate available contacts and existing members
        const availableContacts = contactList.filter(contact => 
          !existingMemberIds.includes(contact.id)
        );
        
        const existingMemberContacts = contactList.filter(contact => 
          existingMemberIds.includes(contact.id)
        );

        setContacts(availableContacts);
        setExistingMembers(existingMemberContacts);
      }
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let message = 'Members processed successfully!';
      if (addedDirectly > 0) {
        ...
        <form onSubmit={handleSubmit} className="flex flex-col">
          <div className="flex-1 overflow-y-auto p-6 space-y-6 max-h-[70vh]">
            ...
            <div>
              ...
              {showContactSelector && existingMembers.length > 0 && (
                <div className="mb-4 p-4 bg-yellow-50 rounded-xl border border-yellow-200">
                  <h4 className="text-sm font-medium text-yellow-800 mb-3">Already Group Members</h4>
                  <div className="space-y-2 max-h-32 overflow-y-auto">
                    {existingMembers.map(member => (
                      <div
                        key={member.id}
                        className="flex items-center justify-between p-2 rounded-lg bg-yellow-100 border border-yellow-200"
                      >
                        <div className="flex items-center space-x-3">
                          <div className="w-8 h-8 bg-gray-100 rounded-full flex items-center justify-center">
                            {member.avatar_url ? (
                          ...
                          <div>
                            ...
                            <p className="text-xs text-yellow-700">Already a member</p>
                          </div>
                        </div>
